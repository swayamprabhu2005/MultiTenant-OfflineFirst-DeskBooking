...
export const EmployeeRosterPage: React.FC = () => {
  ...
  const loadReferenceData = async () => {
    try {
      ...
      if (branchList.length > 0 && !scopedBranchId) {
        setScopedBranchId(branchList[0].id);
      }
    } catch (err) {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      const body: any = {
        ...
        baseBranchId: scopedBranchId || null,
      };
      ...
      setScopedBranchId(branches.length > 0 ? branches[0].id : '');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center space-x-2">
            ...
            <span>Employee Roster &amp; Branch Admins</span>
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Manage employee access roles, appoint branch administrators, and import bulk CSV rosters.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <span>Add User / Branch Admin</span>
          </button>
        </div>
      </div>
      ...
      {isGlobalOrgAdmin && branches.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center max-w-xl mx-auto space-y-4">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">
            Please register at least one branch location before assigning branch administrators.
          </p>
          <Link
            to="/admin/branches"
            className="inline-block px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-md transition-all"
          >
            Create a Branch
          </Link>
        </div>
          ...
          <div className="px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-4 py-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"
              />
            </div>
          </div>
        ...
        <div className="fixed top-0 left-0 w-full h-full z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <h3 className="text-base font-bold text-slate-900 flex items-center space-x-2">
              ...
              <span>Add User / Branch Administrator</span>
            </h3>
