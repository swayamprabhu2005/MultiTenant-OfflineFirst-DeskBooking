...
export const EmployeeDashboard: React.FC = () => {
  ...
  // One-time base branch selection states
  ...
  useEffect(() => {
    // Only prompt base branch selection for employee roles
    if (user && !user.baseBranchId && (user.role === 'EMPLOYEE' || user.role === 'TECH_LEAD')) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* Main Grid: Reservations & Occupancy */}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    {b.status !== 'CANCELLED' && (
                      <button
                        ...
                        className="p-1 text-slate-400 hover:text-rose-655 rounded transition-colors"
                      >
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 max-h-[250px]">
            {loadingOccupancy ? (
              ...
              occupancy.map((occ: any, i: number) => (
                <div key={occ.id || i} className="p-2.5 bg-slate-50/70 border border-slate-100 rounded-xl">
                  <div>
